...
package object jfx {
	...
	@inline private def lift0[B, A](prop: ObservableValue[B], consInit: Boolean)
	                               (implicit ev: B => A): Stream[IO, Option[A]] =
	...
	: Stream[IO, Option[A]] = lift0[A, A](prop, consInit)

	def lift(prop: ObservableBooleanValue, consInit: Boolean)
	: Stream[IO, Option[Boolean]] = lift1[ObservableBooleanValue, java.lang.Boolean, java.lang.Boolean, Boolean](prop, _.get, consInit)

	def lift(prop: ObservableDoubleValue, consInit: Boolean)
	: Stream[IO, Option[Double]] = lift1[ObservableDoubleValue, Number, java.lang.Double, Double](prop, _.get, consInit)

	def lift(prop: ObservableFloatValue, consInit: Boolean)
	: Stream[IO, Option[Float]] = lift1[ObservableFloatValue, Number, java.lang.Float, Float](prop, _.get, consInit)

	def lift(prop: ObservableIntegerValue, consInit: Boolean)
	: Stream[IO, Option[Int]] = lift1[ObservableIntegerValue, Number, java.lang.Integer, Int](prop, _.get, consInit)

	def lift(prop: ObservableLongValue, consInit: Boolean)
	: Stream[IO, Option[Long]] = lift1[ObservableLongValue, Number, java.lang.Long, Long](prop, _.get, consInit)
	...
	def nodeCellFactory[A, N](prop: ObjectProperty[Callback[ListView[A], ListCell[A]]])
	                         ...
	                         (unsafeUpdate: (Option[A], N) => IO[Unit]): Stream[IO, (Option[A], N)] = for {
	...
	def deferUntilLayout[A](parent: Parent)(f: => IO[A]): IO[A] = FXIO {
				...
				Deferred[F2, Unit].flatMap { halt =>
					haltRef.modify { m =>
						...
						m.get(k) match {
							...
							case Some(last) => updated -> last.complete(()).void
						}
